...
Put `#pragma once` as the first line of all headers. This prevents files from being included more than once in a file. Whilst technically `#pragma once` is compiler specific, it's common enough that it's effectively a standard, and all C++ code on the rover is compiled with `gcc` anyway making it a nonissue.
...
!!! warning
    ...
    You may have already seen the standard include guard pattern of
...
In addition, C++ code in this project is compiled with `-Wshadow`. To allow for situations where you want to have a parameter in a constructor with the same name as a class member (for example, constructor initializer lists), it is acceptable to prefix the _parameter_ name with an underscore as needed.
...
=== "Incorrect"
    ...
    // and other code can also call load_settings_from_file,
    // which could cause *really* bad side effects since you can call an un-declared
    // function as long as it's defined somewhere
...
=== "Incorrect"
    ...
    ```cpp
    rover_libraries::drive // too wordy
    RoverLibraries // wrong case
    ```
...
Smart pointers should be initialised with `std::make_unique` or `std::make_shared` as appropriate - they are drop-in, safer replacements for `new`.
...
Dynamic array allocation can also be almost entirely replaced with C++ STL storage containers such as `std::vector` (especially with functions like `std::vector::emplace_back`, which will construct the variable appended to the vector).
...
The STL (Standard Template Library) is _massive_ in modern C++ and contains a huge number of solutions to common problems - the most common likely being `std::string` and `std::vector`.
