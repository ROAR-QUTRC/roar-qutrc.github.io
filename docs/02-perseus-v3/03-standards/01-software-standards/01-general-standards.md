...
Regardless of the language, constants should be named with `SCREAMING_SNAKE_CASE` and, if possible, marked as constant/immutable. Furthermore, constants should not be prefixed with types or any information other than their name.

!!! important

    In, C++, this means using `constexpr`, or `const` if you can't use `constexpr`. Python has no way of doing this.
...
=== "Incorrect"
    ...
    public:
        int speed(); // ambiguous
        void max_speed(int max_speed); // no action specified only a noun
    };
    ```
...
=== "Incorrect"
    ...
    {
    public:
        int _get_max_speed(); // does not need a prefix
    private:
        // private members are missing _ prefix
        void load_defaults();
...
| C source   | `.c`      |                                                                           |
| C header   | `.h`      |                                                                           |
| C++ source | `.cpp`    | `.cc` is less clear and not as common as `.hpp`                           |
| C++ header | `.hpp`    | Always use `.hpp` not `.h` so it is clear the header is used for c++ code |
| Python     | `.py`     |                                                                           |
